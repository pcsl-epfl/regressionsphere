...
def train_ntk(args):
    # TODO: implement kernel ridge regression (??)
    """
    train the NTK di fcnn with (full-batch) gradient descent on the mse loss.
    :param args: parser arguments.
    :return: results dictionary.
    """
    ...
    loss = MSELoss(alpha=1.)

    # initialize network function
    torch.manual_seed(args.netseed)
    f = FC(args.h, args.d, bias=args.bias).to(device)

    yhat = torch.zeros(args.ptr, device=args.device)
